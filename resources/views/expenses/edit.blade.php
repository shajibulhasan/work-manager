    ...
    <div class="col-md-8 offset-md-2">
        <div class="card">
            <div class="card-header bg-success text-white">
                <h5><i class="fas fa-edit"></i> Edit Expense</h5>
            </div>
            <div class="card-body">
                        ...
                        <select class="form-select @error('expense_category') is-invalid @enderror" 
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->name }}" 
                                        {{ old('expense_category', $expense->expense_category) == $category->name ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('spent_by', $expense->spent_by) }}" required>
                            @error('spent_by')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('spent_at', $expense->spent_at) }}" required>
                            @error('spent_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
